...
// src/components/MobileCatalogView/MobileCatalogView.jsx
"use client";
...
export default function MobileCatalogView({
                                              ...
                                              mobileSliderImages,
                                              featuredProducts,
                                              selectedItemId // Новый пропс для выбранного товара
                                          }) {
    const { lang } = useLanguage()
    const { categoryInfo, collectionInfo, pagination } = data;
    ...
    const [isLoading, setIsLoading] = useState(false);
    const [hasMore, setHasMore] = useState(pagination.currentPage < pagination.totalPages);
    const [itemDetail, setItemDetail] = useState(selectedItemId || null); // Инициализируем с selectedItemId
    const router = useRouter();
    const pathname = usePathname();
    const searchParams = useSearchParams();
    const [sortOption, setSortOption] = useState(searchParams.get('sort') || 'default');
    ...
    // Обновляем itemDetail когда selectedItemId изменяется
    useEffect(() => {
        setItemDetail(selectedItemId || null);
    }, [selectedItemId]);
    ...
    const handleItemClick = (id) => {
        ...
        setItemDetail(itemIdStr);

        // Обновляем URL с параметром item
        const current = new URLSearchParams(Array.from(searchParams.entries()));
        current.set('item', itemIdStr);
        const query = current.toString() ? `?${current.toString()}` : '';
